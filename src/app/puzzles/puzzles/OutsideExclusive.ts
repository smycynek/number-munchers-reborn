...
import { getRandomBetweenBoundsWide, getRandomNaturalNumberSet } from '../sampleRandomValues';
import { toExpressionDataSet, getValidOutsideExclusiveValues } from '../sampleValidValues';
...
export class OutsideExclusive extends Puzzle {
  ...
  public override predicate(choice: ExpressionData): boolean {
    return isOutsideExclusive(choice.value, this.target1.value, this.target2.value);
  }

  public override successDetails(choice: ExpressionTypes): ExpressionTypes[] {
    return [choice, s('is either greater than'), this.target2, s('or less than'), this.target1];
  }

  public override errorDetails(choice: ExpressionTypes): ExpressionTypes[] {
    return [choice, s('is not greater than'), this.target2, s('or less than'), this.target1];
  }
