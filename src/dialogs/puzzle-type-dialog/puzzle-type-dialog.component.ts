...
export class PuzzleTypeDialogComponent implements AfterContentInit {
  ...
  constructor() {
    ...
    this.mainFormGroup.addControl('hiddenPuzzleTypeValidator', this.hiddenPuzzleTypeValidator);
    this.mainFormGroup.addControl('puzzleTypesFormGroup', this.puzzleTypesFormGroup);

    this.puzzleTypesFormGroup.valueChanges.pipe(takeUntilDestroyed()).subscribe((value) => {
  ...
  ngAfterContentInit(): void {
    for (const puzzleTypeString of this.PuzzleTypeStrings) {
      const puzzleTypeStringEnum = PuzzleType[puzzleTypeString as keyof typeof PuzzleType];
      const enabled = this.puzzleTypeService.getPuzzleTypes().has(puzzleTypeStringEnum);
      this.puzzleTypesFormGroup.addControl(puzzleTypeString, new FormControl(enabled));
    }
  ...
  protected cancelAndClose() {
    ...
    const thisDialog = document.getElementById('puzzleTypeDialog') as HTMLDialogElement;
    thisDialog.close();
  }
  ...
  public submit(): void {
    ...
    for (const puzzleTypeName of this.PuzzleTypeStrings) {
      ...
      const puzzleTypeNameEnum = PuzzleType[puzzleTypeName as keyof typeof PuzzleType];
